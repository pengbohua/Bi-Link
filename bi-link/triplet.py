...
class TripletDict:
    ...
    def _load(self, path: str):
        ...
        for ex in examples:
            ...
            tr_key = (ex['tail_id'], ex['relation'])

            self.hr2tails[key].add(ex['tail_id'])
            self.tr2heads[tr_key].add(ex['head_id'])    # inverse relations

        self.triplet_cnt = len(examples)

    def get_descendants(self, h: str, r: str) -> set:
        return self.hr2tails.get((h, r), set())

    def get_ancestors(self, t: str, r: str) -> set:
        return self.tr2heads.get((t, r), set())

...
def construct_mask(row_exs: List, col_exs: List = None) -> torch.tensor:
    ...
    for i in range(num_row):
        ...
        neighbor_ids = train_triplet_dict.get_descendants(head_id, relation)
...
def construct_self_negative_mask(exs: List) -> torch.tensor:
    ...
    head_mask = torch.ones(len(exs))
    tail_mask = torch.ones(len(exs))
    for idx, ex in enumerate(exs):
        head_id, relation, tail_id = ex.head_id, ex.relation, ex.tail_id
        descendent_ids = train_triplet_dict.get_descendants(head_id, relation)      # head -> relation (one hop) -> head
        ancestor_ids = train_triplet_dict.get_ancestors(tail_id, relation)
        if head_id in descendent_ids:
            head_mask[idx] = 0
        if tail_id in ancestor_ids:
            tail_mask[idx] = 0

    return {"head_mask": head_mask.bool(), "tail_mask": tail_mask.bool()}

...
relation2ids = {v: i for i, v in enumerate(kg_dict.values())}
...
class Example:
    ...
    def __init__(self, head_id, relation, tail_id, reverse=False, ent=False, **kwargs):
...
class AntiPhrasisExample:

    def __init__(self, head_id, relation, tail_id, relation_id, reverse=False, ent=False, **kwargs):
        self.head_id = head_id
        self.tail_id = tail_id
        self.relation = relation
        if not ent:
            self.relation_id = torch.tensor(relation_id)
        else:
            self.relation_id = -1
...
def load_data(path: str,
    ...
    for i in range(cnt):
        ...
        if add_forward_triplet:
            ...
            # examples.append(AntiPhrasisExample(**obj))      # antiphrasis probing

        if add_backward_triplet:
            examples.append(Example(**reverse_triplet(obj), reverse=True))
        data[i] = None

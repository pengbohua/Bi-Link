...
parser = argparse.ArgumentParser(description='SimKGC-prefix arguments')
...
parser.add_argument('--test-path', default='data/FB15k237/test.txt.json', type=str, metavar='N',
                    help='path to test data')
parser.add_argument('--rel-path', default='data/FB15k237/relations.json', type=str, metavar='N',
...
parser.add_argument('--num-rels', default=237, type=int, metavar='N',
                    help='number of relations in the dataset')
...
parser.add_argument('--template-path', default="templates/",
                    type=str, help='template path'
                    )
parser.add_argument('--prefix-hidden-size', default=512,
                    type=int, help='prefix hidden size'
                    )
parser.add_argument('--compress-len', default=4,
                    type=int, help='keep the len of entity document to the second stage'
                    )
parser.add_argument('--rel-len', default=20,
                    type=int, help='the length of relation expression'
                    )
parser.add_argument('--prefix-seq-len', default=40, type=int, help='prefix for relational expression')
parser.add_argument('--prefix-projection', default=True, type=bool, help='project prefix with a mlp or embedding')
parser.add_argument('--feat-comp', default=True, help='Use feat compression')
...
parser.add_argument('--eval-model-path', default='checkpoint', type=str, metavar='N',
                    help='path to model, only used for evaluation')
parser.add_argument("--do-mlm", default=False, type=bool,
                    help='num of layers to freeze in bert encoder')
parser.add_argument("--mlm_weight", default=0.1, type=float,
                    help='weight for mlm loss')

args = parser.parse_args()
# print("train", args.train_path)
# print("valid", args.valid_path)
print(args)
...
if args.model_dir:
    args.model_dir = os.path.join(args.model_dir, curr_time)
...
print("parsing configuration finished")